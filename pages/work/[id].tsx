    ...
    return (
        <article className="relative flex flex-col mx-auto max-w-screen-2xl w-11/12 md:w-8/12 mt-36 md:mt-20 mb-16">
            ...
            <section id="work-header" className="flex flex-col mx-auto text-center">
                ...
                <h2 className="hidden mt-8 md:inline-block font-roboto font-thin mt-3 text-5xl">
                    {workData.type}
                </h2>
            </section>
            {workData.src &&
            ...
            mx-auto mt-6 md:mt-16 w-11/12 md:divide-white divide-x divide-solid">
                <div id="info" className="font-roboto pl-2 pr-4 pt-4 pb-4 text-white box-border
                pl-2 pr-4 bg-info-box rounded-md w-full lg:w-4/12 pr-8 text-2xl">
                    {workData.url &&
                        <div id="url" className="w-full inline-block">
                            <p className="font-bold text-xl mt-4 pl-1 sm:text-3xl md:text-2xl">URL</p>
                            <a href={workData.url} target="_blank">
                                <span className="font-light underline ml-2 mb-4 text-sm overflow-ellipsis inline-block">
                                    {workData.url}
                                </span>
                            </a>
                        </div>}
                    {workData.repository &&
                        <div id="repo" className="w-full inline-block">
                            <p className="font-bold text-xl pl-1 mt-4 sm:text-3xl md:text-2xl">Repository</p>
                            <a href={workData.repository} target="_blank">
                                <span className="font-light underline ml-2 mb-4 mt-1 text-sm overflow-ellipsis w-full block">
                                    {workData.repository}
                                </span>
                            </a>
                        </div>}
                        ...
                        <div id="participants">
                            ...
                            <p className="ml-2">
                                {workData.participants.map((participant: string, i: number) =>
                                    <span key={i} className="font-light text-sm sm:text-xl mt-1">{participant}</span>)}
                            </p>
                        </div>}
                    <div id="technologies-used">
                        ...
                        <div className="ml-2">
                            {workData.technologies.map((technology: string, i: number) =>
                                <span key={i} className="font-light text-sm sm:text-xl inline-block">{technology}</span>)}
                        </div>
                    </div>
                    <div id="duration" className="hidden lg:inline-block">
                        ...
                        <div className="ml-2 font-light text-xl inline-block">
                            {workData.duration}
                        </div>
                    </div>
                </div>
                <div id="detail" className="box-border w-full lg:w-9/12 pl-2 pr-4 py-4
                bg-white rounded-md font-roboto text-lg sm:text-2xl lg:text-xl md:pl-8">
                    <p className="font-bold text-xl mt-4 pl-1 sm:text-3xl md:text-2xl">Description</p>
                    <p className="font-light mt-2 mb-4 ml-4 md:ml-2">{workData.description}</p>
                    {workData.roles &&
                        (<>
                            <p className="font-bold text-xl pl-1 sm:text-3xl md:text-2xl mt-8">Contributions</p>
                            <div className="font-light mt-4 mb-4 ml-8 md:ml-4">
                                <ul className="list-disc">
                                    {workData.roles.map((role, i) => {
                                        return (
                                            <li key={i} className="ml-2 mb-2">
                                                {role}
                                            </li>
                                        )
                                    })}
